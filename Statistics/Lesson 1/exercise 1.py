# File "bank_customers.csv" that you can find at "https://raw.githubusercontent.com/kupav/data-sc-intro/main/data/" describes bank clinets.
# It contains three columns: customer age (CustAge), period of relationship with the bank in months (Months),
# and credit limit (CredLim).
# Plot a histogram for the column Month and compute its mean, median and the mode.
# Show these values on the histogram. Make a second copy of the histogram,
# compute quartiles and show them on the plot.
import requests
...
print(lst_data[:10])
# Take only months
data_grd = [int(s[1]) for s in lst_data]
print(data_grd[:10])
